...
%% 数据预筛  可选，可不选
n = 1000;   %划分段落数目
segment_length = floor(size(s_d,1)/n);
s_d_segment = cell(n,1);
for i = 1:n
    % 计算当前段的起始和结束索引
    start_idx = (i - 1) * segment_length + 1;
    if i == n
        end_idx = size(s_d, 1);  % 最后一段包含剩余的部分
    else
        end_idx = i * segment_length;
    end
    
    % 分段存储，确保每段保持 3 列
    s_d_segment{i} = s_d(start_idx:end_idx, :);
end
...
% Fs = 1;
% for i = 1:n
%     segmentData = s_d_segment{i};
%     [Pxx,~] = pwelch(segmentData(:,1),[],[],[],Fs);
%     [Pyy,~] = pwelch(segmentData(:,2),[],[],[],Fs);
%     [Pzz,~] = pwelch(segmentData(:,3),[],[],[],Fs);
%     Sdx(i) = std(Pxx);
%     Sdy(i) = std(Pyy);
%     Sdz(i) = std(Pzz);
% end
% SdStd = [Sdx, Sdy, Sdz];
for i = 1:n
    segmentData = s_d_segment{i};  % 提取每一段数据

    % 直接计算每个维度数据的标准差
    Sdx(i) = std(segmentData(:,1));  % X 轴的标准差
    Sdy(i) = std(segmentData(:,2));  % Y 轴的标准差
    Sdz(i) = std(segmentData(:,3));  % Z 轴的标准差
end

SdStd = [Sdx, Sdy, Sdz];  % 将标准差值保存到一个矩阵中


% 对每个方向的标准差进行排序，返回排序后的值和索引
[Sdx_sorted, idx_Sdx_sorted] = sort(Sdx, 'descend');
[Sdy_sorted, idx_Sdy_sorted] = sort(Sdy, 'descend');
[Sdz_sorted, idx_Sdz_sorted] = sort(Sdz, 'descend');

% 选择前10大的值及其索引
%top_Sdx_values = Sdx_sorted(1:3);
idx_Sdx = idx_Sdx_sorted(1:2);  % 修改为选择前10个索引

%top_Sdy_values = Sdy_sorted(1:3);
idx_Sdy = idx_Sdy_sorted(1:2);  % 修改为选择前10个索引

%top_Sdz_values = Sdz_sorted(1:3);
idx_Sdz = idx_Sdz_sorted(1:2);  % 修改为选择前10个索引

% 组合这些索引，去除重复项
indices_to_delete = unique([idx_Sdx, idx_Sdy, idx_Sdz]);

% 从 s_d_segment 中删除对应的段落
s_d_segment(indices_to_delete) = [];  % 删除对应的段落
% 假设 s_d_segment 是一个 3x1 的 cell 数组，每个 cell 中包含一个 12583x3 的矩阵
s_d = cell2mat(s_d_segment);  % 将 cell 数组中的所有矩阵合并成一个大的矩阵


%% 数据预筛  可选，可不选

segment_length_b = floor(size(s_b,1)/n);
s_b_segment = cell(n,1);
for i = 1:n
    % 计算当前段的起始和结束索引
    start_idx = (i - 1) * segment_length_b + 1;
    if i == n
        end_idx = size(s_b, 1);  % 最后一段包含剩余的部分
    else
        end_idx = i * segment_length_b;
    end
    
    % 分段存储，确保每段保持 3 列
    s_b_segment{i} = s_b(start_idx:end_idx, :);
end
s_b_segment(indices_to_delete) = [];  % 从 s_d_segment 中删除对应的段落
% 假设 s_d_segment 是一个 3x1 的 cell 数组，每个 cell 中包含一个 12583x3 的矩阵
s_b = cell2mat(s_b_segment);  % 将 cell 数组中的所有矩阵合并成一个大的矩阵


%%
%     figure;
%     hold on;
%     plot(t_d,v_d(:,1),'r',LineWidth=0.5);
%     plot(t_d,v_d(:,2),'b',LineWidth=0.5);
%     plot(t_d,v_d(:,3),'g',LineWidth=0.5);
%     xlabel('Time (s)');
...
% figure;
% hold on;
% plot(t_b,v_b(:,1),'r',LineWidth=1.5);
% plot(t_b,v_b(:,2),'b',LineWidth=1.5);
% plot(t_b,v_b(:,3),'g',LineWidth=1.5);
% xlabel('Time (s)');
% ylabel('Displacement (m)');
...
% 初始化角度记录数组
angles = zeros(length(s_b), 1);  % 创建一个与时间序列长度相同的数组用于存储角度
...
plot(t, anglesOrigin, 'b', 'LineWidth', 1.5);  % Plot the first set of angles (blue line)
hold on;
t2 = 1:length(angles);  % Ensure the x-axis matches the length of the second angles array
plot(t2, angles, 'r', 'LineWidth', 1.5);  % Plot the second set of angles (red line)
xlabel('Time (s)');
...
% Sb1 = Sb(:, end);  % Sb的最后一列
% Sb2 = sum(Sb(:, end-50000:end-10000), 2);  % Sb的其他列的和
% Sd1 = Sd(:, end);  % Sd的最后一列
% Sd2 = sum(Sd(:, end-50000:end-10000), 2);  % Sd的其他列的和
% % Sb1 = sum(Vb,2);
% % Sb2 = sum(Vb(:,201:end-200), 2);
% % Sd1 = sum(Vd,2);  % Sb 的最后一列
% % Sd2 = sum(Sd(:,201:end-200), 2);
%
%
%
% % 计算 Sb1 和 Sb2 的叉积
